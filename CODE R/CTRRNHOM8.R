...
cau1 <- read.xlsx("C:/BTL/file/BTLCTRR.xlsx",sheet ="TONG HOP")
soLuong = nrow(cau1)
View(soLuong)

#cau 2
data2<-read.xlsx("C:/BTL/file/BTLCTRR.xlsx",sheet = "TONG HOP")
i<-{1:196}
k<-{2:27}
...
#cau3
cau3<-read.xlsx("C:/BTL/file/BTLCTRR.xlsx",sheet = "TONG HOP")
dulieu <- data.frame(cau3[1:196,2:27])
sum= c(rowSums(dulieu))
SV = data.frame(cau3[,1])
colnames(SV) <- c("SV")
Trung_Binh_SV = data.frame(SV, GTTB = round(sum/26,digits = 2))
View(Trung_Binh_SV)

#cau4
cau4<-read.xlsx("C:/BTL/file/BTLCTRR.xlsx",sheet = "TONG HOP")
...
#cau5
cau5<-read.xlsx("C:/BTL/file/BTLCTRR.xlsx",sheet = "TONG HOP")
dulieu <- data.frame(cau5[1:196,2:27])
tongdiemTC_SV = (rowSums(dulieu))
maximum = max(tongdiemTC_SV)
svNameMax = c()
findMax = c()
k = 1
for( i in 1:196){
  if(tongdiemTC_SV[i] == maximum){
    findMax[k] = tongdiemTC_SV[i]
    svNameMax[k] = Trung_Binh_SV[i,1]
    k = k+1
  }
}
SV_Co_tong_diem_lon_nhat = data.frame(Ten_SVmax = svNameMax , Tong_diemmax= findMax)

#cau6 

data6<-read.xlsx("C:/BTL/file/BTLCTRR.xlsx",sheet = "TONG HOP")
i<-{1:196}
k<-{2:27}
...
#cau 10
data10<-read.xlsx("C:/BTL/file/BTLCTRR.xlsx",sheet = "TONG HOP")
i<-{1:196}
k<-{2:27}
...
#cau 11
data11<-read.xlsx("C:/BTL/file/BTLCTRR.xlsx",sheet = "TONG HOP")
i<-{1:196}
k<-{2:27}
...
len <- length(e1_c19[,1]); tc <- data.frame(c(2, 8, 7, 7, 4, 5, 3), c(15, 26, 22, 23, 13, 17, 20));
for (i in 1: len) {
  for (j in 1:7) {
...
slsv_e1_c19 <- c();
for (i in 1:7) {
  slsv_e1_c19[i]  <- length(hieu_e1_c19[hieu_e1_c19[,i]>=2,i])
}

hieu_e2_c19 <- data.frame();
len <- length(e2_c19[,1]); tc <- data.frame(c(2, 8, 7, 7, 4, 5, 3), c(15, 26, 22, 23, 13, 17, 20));
for (i in 1: len) {
  for (j in 1:7) {
...
len <- length(e5_c19[,1]); tc <- data.frame(c(2, 8, 7, 7, 4, 5, 3), c(15, 26, 22, 23, 13, 17, 20));
for (i in 1: len) {
  for (j in 1:7) {
...
len <- length(e6_c19[,1]); tc <- data.frame(c(2, 8, 7, 7, 4, 5, 3), c(15, 26, 22, 23, 13, 17, 20));
for (i in 1: len) {
  for (j in 1:7) {
...
len <- length(e7_c19[,1]); tc <- data.frame(c(2, 8, 7, 7, 4, 5, 3), c(15, 26, 22, 23, 13, 17, 20));
for (i in 1: len) {
  for (j in 1:7) {
...
len <- length(e8_c19[,1]); tc <- data.frame(c(2, 8, 7, 7, 4, 5, 3), c(15, 26, 22, 23, 13, 17, 20));
for (i in 1: len) {
  for (j in 1:7) {
...
#cau 20
cau20<-read.xlsx("C:/BTL/file/BTLCTRR.xlsx",sheet = "TONG HOP")
dulieu <- data.frame(cau20[1:196,2:27])

diemTongTC = c()

for (i in 1:26){
  
  values= sum(dulieu[1:196,i])
  
  diemTongTC[i] <- values
  
}

hist(diemTongTC,xlab="Diem",ylab="Tan Suat",main="Bieu do hien thi phan phoi tong diem cac tieu chi cua SV",ylim=c(0,12),col="blue")

#cau21
cau21<-read.xlsx("C:/BTL/file/BTLCTRR.xlsx",sheet = "TONG HOP")
dulieu <- data.frame(cau21[1:196,2:27])
sum= c(rowSums(dulieu))
tong_diem_SV = data.frame(SV = cau21[1:196,1], tongDiemSV = sum)
barplot(tong_diem_SV$tongDiemSV,names.arg=tong_diem_SV$SV,xlab="Sinh
Vien",ylab="Tong Diem",cex.names=0.5,ylim=c(0,120),col = "blue")

#cau 22

data22<-read.xlsx("C:/BTL/file/BTLCTRR.xlsx",sheet = "TONG HOP")
i<-{1:196}
k<-{2:27}
...
s1<-data.frame(sheet1[,8],sheet1[,2],sheet1[,3],
               sheet1[,4],sheet1[,5],sheet1[,11],
               sheet1[,13],sheet1[,14],sheet1[,15],
               sheet1[,16],sheet1[,17],sheet1[,19],sheet1[,22])
s2<-data.frame(sheet2[,8],sheet2[,2],sheet2[,3],
               sheet2[,4],sheet2[,5],sheet2[,11],
               sheet2[,13],sheet2[,14],sheet2[,15],
               sheet2[,16],sheet2[,17],sheet2[,19],sheet2[,22])
s5<-data.frame(sheet5[,8],sheet5[,2],sheet5[,3],
               sheet5[,4],sheet5[,5],sheet5[,11],
               sheet5[,13],sheet5[,14],sheet5[,15],
               sheet5[,16],sheet5[,17],sheet5[,19],sheet5[,22])
s6<-data.frame(sheet6[,8],sheet6[,2],sheet6[,3],
               sheet6[,4],sheet6[,5],sheet6[,11],
               sheet6[,13],sheet6[,14],sheet6[,15],
               sheet6[,16],sheet6[,17],sheet6[,19],sheet6[,22])
s7<-data.frame(sheet7[,8],sheet7[,2],sheet7[,3],
               sheet7[,4],sheet7[,5],sheet7[,11],
               sheet7[,13],sheet7[,14],sheet7[,15],
               sheet7[,16],sheet7[,17],sheet7[,19],sheet7[,22])
s8<-data.frame(sheet8[,8],sheet8[,2],sheet8[,3],
               sheet8[,4],sheet8[,5],sheet8[,11],
               sheet8[,13],sheet8[,14],sheet8[,15],
               sheet8[,16],sheet8[,17],sheet8[,19],sheet8[,22])

sum1=sum2=sum5=sum6=sum7=sum8=0;
for(i in 1:30) sum1=sum1+sum(s1[i,]) 
tb1=signif(sum1/(30*13),digits = 3)
for(i in 1:26) sum2=sum2+sum(s2[i,]) 
tb2=signif(sum2/(26*13),digits = 3)
for(i in 1:35) sum5=sum5+sum(s5[i,]) 
tb5=signif(sum5/(35*13),digits = 3)
for(i in 1:27) sum6=sum6+sum(s6[i,]) 
tb6=signif(sum6/(27*13),digits = 3)
for(i in 1:40) sum7=sum7+sum(s7[i,]) 
tb7=signif(sum7/(40*13),digits = 3)
for(i in 1:38) sum8=sum8+sum(s8[i,]) 
tb8=signif(sum8/(38*13),digits = 3)
tb<-c(tb1,tb2,tb5,tb6,tb7,tb8)
tt<-c(1,2,5,6,7,8)
plot(x=tt, y=tb,ylab = "TB tieu chi ky thuat theo Sheet",
     ylim = c(2.2,3), main = "Ve Scatterplot tung sheet theo tieu chi ky thuat",
     xlab = "Sheet")

#cau 27

sheet1<-read.xlsx("C:/BTL/file/BTLCTRR.xlsx",sheet = "E1")
sheet2<-read.xlsx("C:/BTL/file/BTLCTRR.xlsx",sheet = "E2")
sheet5<-read.xlsx("C:/BTL/file/BTLCTRR.xlsx",sheet = "E5")
sheet6<-read.xlsx("C:/BTL/file/BTLCTRR.xlsx",sheet = "E6")
sheet7<-read.xlsx("C:/BTL/file/BTLCTRR.xlsx",sheet = "E7")
sheet8<-read.xlsx("C:/BTL/file/BTLCTRR.xlsx",sheet = "E8")
...
kt6sheet <- read.xlsx("C:/BTL/file/BTLCTRR.xlsx", sheet = "KT")
kt6sheet_final <- data.frame(kt6sheet[1:196,2:14])
f_ktsheet1 = data.frame(table(kt6sheet_final[,1]))
f_ktsheet2 = data.frame(table(kt6sheet_final[,2]))
f_ktsheet3 = data.frame(table(kt6sheet_final[,3]))
f_ktsheet4 = data.frame(table(kt6sheet_final[,4]))
f_ktsheet5 = data.frame(table(kt6sheet_final[,5]))
f_ktsheet6 = data.frame(table(kt6sheet_final[,6]))
f_ktsheet7 = data.frame(table(kt6sheet_final[,7]))
f_ktsheet8 = data.frame(table(kt6sheet_final[,8]))
f_ktsheet9 = data.frame(table(kt6sheet_final[,9]))
f_ktsheet10 = data.frame(table(kt6sheet_final[,10]))
f_ktsheet11 = data.frame(table(kt6sheet_final[,11]))
f_ktsheet12 = data.frame(table(kt6sheet_final[,12]))
f_ktsheet13 = data.frame(table(kt6sheet_final[,13]))
freq <-c()
for ( i in 1:nrow(f_ktsheet1)){
  freq[i] <- f_ktsheet1[i,2] + f_ktsheet2[i,2] +
    f_ktsheet3[i,2]+ f_ktsheet4[i,2]+ f_ktsheet5[i,2]+
    f_ktsheet6[i,2]+ f_ktsheet7[i,2]+ f_ktsheet8[i,2]+ 
    f_ktsheet9[i,2]+ f_ktsheet10[i,2]+ f_ktsheet11[i,2]+
    f_ktsheet12[i,2]+ f_ktsheet13[i,2]
}
...
for (i in 1:length(freq)) {
  f_tuongdoi[i] <- freq[i]/sum_f
}

f_tuongDoiTichLuy <- c()
f_tuongDoiTichLuy[1]<-f_tuongdoi[1]
for ( i in 2:length(f_tuongdoi)){
  f_tuongDoiTichLuy[i] = f_tuongDoiTichLuy[i-1]+f_tuongdoi[i]
}

diem <- f_ktsheet1[,1]

dotchart(f_tuongDoiTichLuy, labels = diem,
         cex = 1.1, xlab = "Tan So Tuong Doi Tich Luy",
         main = "bieu do tan so tich luy ve ky thuat")
lines(f_tuongDoiTichLuy,y=1:6,col="red")

f_table <- data.frame(diem,freq,f_tuongdoi, f_tuongDoiTichLuy)

#cau 30

data30<-read.xlsx("C:/BTL/file/BTLCTRR.xlsx",sheet = "TONG HOP")
i<-{1:196}
k<-{2:27}
...
plot(m_c32, xlab="E1-E2-E5-E6-E7-E8",
     ylab="Cap tieu chi tuong ung",
     main ="So luong sinh vien sai lech lon")
